...
describe("Completion", () => {
  ...
  context("attributes", () => {
    it("proposes all attributes", () => {
      const completions = completion.completeTodoAttribute("This @", 6)
      should(completions).deepEqual(ctx.parsedFolder.attributes)
    })
    it("proposes corresponding attributes", () => {
      const completions = completion.completeTodoAttribute("This @attr", 10)
    ...
    it("proposes corresponding attribute", () => {
      const completions = completion.completeTodoAttribute("This @du", 8)
    ...
    it("ignores characters after the carret", () => {
      const completions = completion.completeTodoAttribute("This @de", 7)
      should(completions).not.containEql("attribute1")
      should(completions).not.containEql("attribute2")
      should(completions).containEql("due")
    })
    it("proposes nothing if nothing matches", () => {
      const completions = completion.completeTodoAttribute("This @de", 8)
      should(completions).be.empty()
    })
    it("doesn't propose for previous line", () => {
      const completions = completion.completeTodoAttribute("This @d\nu", 9)
      should(completions).be.empty()
    })
  })
  context("attribute values", () => {
    it("proposes all values for a given attribute", () => {
      const completions = completion.completeTodoAttribute("This @attribute1(", 17)
    ...
    it("proposes values for a given attribute starting with the right letters", () => {
      const completions = completion.completeTodoAttribute("This @attribute1(va", 19)
      should(completions).deepEqual(["value1", "value2"])
    })
    it("ignores what's written after", () => {
      const completions = completion.completeTodoAttribute("This @attribute1(var", 19)
      should(completions).deepEqual(["value1", "value2"])
    })
    it("doesn't propose values for an unknown attribute", () => {
      const completions = completion.completeTodoAttribute("This @something(", 16)
      should(completions).be.empty()
    })
    it("doesn't propose values for an attribute without values", () => {
      const completions = completion.completeTodoAttribute("This @attribute2(", 17)
      should(completions).be.empty()
    })
    it("doesn't propose values for something that looks like an attribute but isn't one", () => {
      const completions = completion.completeTodoAttribute("This attribute1(", 16)
      should(completions).be.empty()
    })
  })
})