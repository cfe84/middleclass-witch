...
export class FileInspector {
  ...
  inspect(file: string): InspectionResults {
    ...
    if (splat.length > 1 && (containingFolderType === "current" || containingFolderType === "recurrences")) {
      project = splat.shift() || ""
    }
    let name = ""
    if (splat.length > 0) {
    ...
    if ((containingFolderType === "current" || containingFolderType === "recurrences") && !project && name) {
